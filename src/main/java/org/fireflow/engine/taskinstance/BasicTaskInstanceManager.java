...
//import com.lvbang.framework.SpringContext;
//import com.lvbang.wfmanage.service.WfToDoService;
...
//	private WfToDoService wfToDoServiceImpl;
	...
	public final void createTaskInstances(IToken token,
		...
		for (int i = 0; i < activity.getTasks().size(); i++) {
			...
			/**
			 * 李强20140718新增开始，为站级运维特殊时限限制制定，不同级别的故障预警时长也不一样
			 */
//			if(duration==null){
//				wfToDoServiceImpl=(WfToDoService) SpringContext.getBean("wfToDoServiceImpl");
//				duration=wfToDoServiceImpl.handleDuration((TaskInstance)taskInstance);
//			}
			/**
			 * 李强20140718新增结束
			 */
...
//		}
	...
	public IWorkItem withdrawWorkItem(IWorkItem workItem)
		...
		try {
			...
			/**
			 * 李强20140607修改，通过流程实例id、任务id，操作员查询待办工单
			 */
			List<IWorkItem>  workItems = persistenceService.findTodoWorkItemsByWithdraw(workItem
					.getActorId(), workItem.getTaskInstance().getProcessInstanceId(),
					workItem.getTaskInstance().getTaskId());
