...
//    public static final String hours_of_business_week = "hours_of_business_week";
    ...
    private List<WfVacation> list = null;
...
//判断date属于哪个时间段
    ...
    public boolean isBusinessDay(Date d) {
        ...
        if(list==null){
        	...
        	list = rtCtx.getPersistenceService().findWfVacationByYear(sf.format(new Date()));
        }
        
        if(list!=null && list.size()>0){
        	SimpleDateFormat sf2=new SimpleDateFormat("yyyy-MM-dd");
        	for(WfVacation wfVacation:list){
        		if(sf2.format(d).equals(sf2.format(wfVacation.getVacationDate()))){
        			return false;
        		}
        	}
        }
