...
//import com.lvbang.itsm.workflow.FlowDataOfBuz;
...
//	private static Log log = LogFactory.getLog(PersistenceServiceSpringJdbcImpl.class);
	...
	private Logger logger=org.apache.log4j.LogManager.getLogger(PersistenceServiceSpringJdbcImpl.class);
	...
	public void saveOrUpdateProcessInstance(IProcessInstance processInstance)
	{
		...
		if (processInstance.getId() == null || processInstance.getId().trim().equals(""))
		{
			...
			if (show_sql)
			{
				logger.debug("FIREWORKFLOW_JDBC:" + sql.toString());
				logger.debug("Parameters:"+processInstanceId+","+processInstance.getProcessId()+","+processInstance.getVersion()+","+
						processInstance.getName()+","+processInstance.getDisplayName()+","+processInstance.getState()+","+(processInstance.isSuspended() == true ? 1 : 0)+","+
						...
						processInstance.getParentProcessInstanceId()+","+ processInstance.getParentTaskInstanceId()+","+processInstance.getBuzCode());
				//System.out.println("FIREWORKFLOW_JDBC:" + sql.toString());
			}
...
//			processInstanceId = java.util.UUID.randomUUID().toString().replace("-", "");
		...
		else
		{
			...
			{
				logger.debug("FIREWORKFLOW_JDBC:" + sql.toString());
				logger.debug("Parameters:"+processInstance.getProcessId()+","+ processInstance.getVersion()+","+
						...
						processInstance.getParentTaskInstanceId()+","+processInstance.getBuzCode()+","+ processInstance.getId());
				//System.out.println("FIREWORKFLOW_JDBC:" + sql.toString());
			}
	...
	public void saveOrUpdateTaskInstance(ITaskInstance taskInstance)
	{
		...
		if (taskInstance.getId() == null || taskInstance.getId().trim().equals(""))
		{
			...
			{
				logger.debug("FIREWORKFLOW_JDBC:" + sql.toString());
				logger.debug("Parameters:"+taskInstanceId+","+
							taskInstance.getClass().getName()+","+
							...
							((TaskInstance) taskInstance).getFromActivityId()+","+ taskInstance.getStepNumber()+","+(((TaskInstance) taskInstance).getCanBeWithdrawn() == true ? 1 : 0));
				//System.out.println("FIREWORKFLOW_JDBC:" + sql.toString());
			}
...
//			taskInstanceId = java.util.UUID.randomUUID().toString().replace("-", "");
		...
		else
		{
			...
			{
				logger.debug("FIREWORKFLOW_JDBC:" + sql.toString());
				logger.debug("Parameters:"+taskInstance.getClass().getName()+","+
											taskInstance.getTaskId()+","+ taskInstance.getActivityId()+","+ taskInstance.getName()+","+
	...
	public void saveOrUpdateWorkItem(IWorkItem workitem)
	{
		...
		if (workitem.getId() == null || workitem.getId().trim().equals(""))
		{
			...
			{
				logger.debug("FIREWORKFLOW_JDBC:" + sql.toString());
				logger.debug("Parameters:"+workItemId+","+ workitem.getState()+","+ getSqlDateTime(workitem.getCreatedTime())+","+
...
//			workItemId = java.util.UUID.randomUUID().toString().replace("-", "");
		...
		else
		{
			...
			{
				logger.debug("FIREWORKFLOW_JDBC:" + sql.toString());
				logger.debug("Parameters:"+workitem.getState()+","+ getSqlDateTime(workitem.getCreatedTime())+","+
						getSqlDateTime(workitem.getClaimedTime())+","+ getSqlDateTime(workitem.getEndTime())+","+
	...
	public void saveOrUpdateToken(IToken token)
	{
		...
		if (token.getId() == null || token.getId().trim().equals(""))
		{
			...
			{
				logger.debug("FIREWORKFLOW_JDBC:" + sql.toString());
				logger.debug("Parameters:"+tokenId+","+ (token.isAlive() == true ? 1 : 0)+","+ token.getValue()+","+ token.getNodeId()+","+
...
//			tokenId = java.util.UUID.randomUUID().toString().replace("-", "");
		...
		else
		{
			...
			{
				logger.debug("FIREWORKFLOW_JDBC:" + sql.toString());
				logger.debug("Parameters:"+(token.isAlive() == true ? 1 : 0)+","+ token.getValue()+","+ token.getNodeId()+","+ token.getProcessInstanceId()+","+
						token.getStepNumber()+","+
	...
	public Integer getAliveTokenCountForNode(final String processInstanceId, final String nodeId)
	{
		...
		if (show_sql)
		{
			logger.debug("FIREWORKFLOW_JDBC:select count(*) from T_FF_RT_TOKEN where alive=1 and processinstance_id=? and node_id =?");
			logger.debug("Parameters:"+ processInstanceId+","+nodeId);
		}
	...
	public Integer getCompletedTaskInstanceCountForTask(final String processInstanceId, final String taskId)
	{
		if (show_sql)
		{
			logger.debug("FIREWORKFLOW_JDBC:"+"select count(*) from T_FF_RT_TASKINSTANCE where state=" + ITaskInstance.COMPLETED
					+ " and task_id=? and processinstance_id=? ");
			logger.debug("Parameters:"+ taskId+","+processInstanceId);
		}
	...
	public Integer getAliveTaskInstanceCountForActivity(final String processInstanceId, final String activityId)
	{
		if (show_sql)
		{
			logger.debug("FIREWORKFLOW_JDBC:"+"select count(*) from T_FF_RT_TASKINSTANCE where " + " (state=" + ITaskInstance.INITIALIZED
					+ " or state=" + ITaskInstance.RUNNING + ")" + " and activity_id=? and processinstance_id=? ");
			logger.debug("Parameters:"+ activityId+","+processInstanceId);
		}
			...
			final String activityId)
	{
		...
		if (activityId != null && !activityId.trim().equals(""))
		{
			...
			if (show_sql)
			{
				logger.debug("FIREWORKFLOW_JDBC:"+sb.toString()+orderBy);
				logger.debug("Parameters:"+ processInstanceId+","+activityId);
			}
		...
		else
		{
			...
			{
				logger.debug("FIREWORKFLOW_JDBC:"+sb.toString()+orderBy);
				logger.debug("Parameters:"+ processInstanceId);
			}
			...
			final Integer stepNumber)
	{
		...
		if (stepNumber != null)
		{
			...
			{
				logger.debug("FIREWORKFLOW_JDBC:"+sb.toString()+orderBy);
				logger.debug("Parameters:"+ processInstanceId+","+stepNumber);
			}
...
//		 String sql =
//		 "select * from t_ff_rt_taskinstance with (updlock, rowlock) where id=? ";//geng
		// 以下是oracle中的语句
		...
		super.getJdbcTemplate().batchUpdate("delete from t_ff_rt_token where processinstance_id = ? and node_id=? ",
				new BatchPreparedStatementSetter()
				{
					public void setValues(PreparedStatement ps, int i) throws SQLException
					{
						if (show_sql)
						{
							logger.debug("FIREWORKFLOW_JDBC:"+"delete from t_ff_rt_token where processinstance_id = ? and node_id=? ");
							logger.debug("Parameters:"+ processInstanceId+","+nodeIdsList.get(i));
						}
	...
	public void deleteTokensForNode(final String processInstanceId, final String nodeId)
	{
		...
		if (show_sql)
		{
			logger.debug("FIREWORKFLOW_JDBC:"+sql);
			logger.debug("Parameters:"+ processInstanceId+","+nodeId);
		}
	...
	public void deleteToken(IToken token)
	{
		...
		{
			logger.debug("FIREWORKFLOW_JDBC:"+sql);
			logger.debug("Parameters:"+token.getId());
		}
	...
	public IWorkItem findWorkItemById(String id)
	{
		...
		if (show_sql)
		{
			logger.debug("FIREWORKFLOW_JDBC:"+workItemSql);
			logger.debug("Parameters:"+id);
		}
		...
		if (show_sql)
		{
			logger.debug("FIREWORKFLOW_JDBC:"+taskInstanceSql);
			logger.debug("Parameters:"+workItem.getTaskInstanceId());
		}
	...
	public void abortTaskInstance(final TaskInstance taskInstance)
	{
		...
		{
			logger.debug("FIREWORKFLOW_JDBC:"+sql);
			logger.debug("Parameters:"+ITaskInstance.CANCELED+","+getSqlDateTime(rtCtx.getCalendarService().getSysDate())+","+taskInstance.getId());
		}
		...
		{
			logger.debug("FIREWORKFLOW_JDBC:"+workItemSql);
			logger.debug("Parameters:"+rtCtx.getCalendarService().getSysDate()+","+taskInstance.getId());
		}
	...
	public List<IWorkItem> findWorkItemsForTask(final String taskid)
	{
		...
		{
			logger.debug("FIREWORKFLOW_JDBC:"+sql);
			logger.debug("Parameters:"+taskid);
		}
		...
		else
		{
			...
			for (IWorkItem workItem : l)
			{
				...
				{
					logger.debug("FIREWORKFLOW_JDBC:"+taskInstanceSql);
					logger.debug("Parameters:"+ ((WorkItem) workItem).getTaskInstanceId());
				}
	...
	public List<IProcessInstance> findProcessInstancesByProcessId(final String processId)
	{
		...
		{
			logger.debug("FIREWORKFLOW_JDBC:"+sql);
			logger.debug("Parameters:"+ processId);
		}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<IProcessInstance> findAllProcessInstances()
	{

		String sql = "select * from t_ff_rt_processinstance order by created_time ";
		...
		List<IProcessInstance> l = new ArrayList<IProcessInstance>();
		l = (List<IProcessInstance>) super.getJdbcTemplate().query(sql,new ProcessInstanceRowMapper());
		if (l == null || l.size() < 1)
		{
			return null;
		}
			...
			final Integer version)
	{
		...
		{
			logger.debug("FIREWORKFLOW_JDBC:"+sql);
			logger.debug("Parameters:"+processId+","+version);
		}
	...
	public List<IWorkItem> findTodoWorkItems(final String actorId)
	{
		...
		String workItemSql = " select a.*,c.buz_code from t_ff_rt_workitem a,t_ff_rt_taskinstance b,t_ff_rt_processinstance c" +
				" where a.taskinstance_id=b.id and b.PROCESSINSTANCE_ID=c.id and (a.state=" + IWorkItem.INITIALIZED + " or a.state="
				+ IWorkItem.RUNNING + " ) and a.actor_id=?  ";

		if (show_sql)
		{
			logger.debug("FIREWORKFLOW_JDBC:" + workItemSql.toString());
			logger.debug("Parameters:"+actorId);
			//System.out.println("FIREWORKFLOW_JDBC:" + sql.toString());
		}
		
		List<IWorkItem> l = (List<IWorkItem>) super.getJdbcTemplate().query(workItemSql, new Object[] { actorId },new WorkItemWithBuzRowMapper());
...
//		return l;
		...
		else
		{
			...
			for (IWorkItem workItem : l)
			{
				if (show_sql)
				{
					logger.debug("FIREWORKFLOW_JDBC:" + taskInstanceSql.toString());
					logger.debug("Parameters:"+((WorkItem) workItem).getTaskInstanceId());
					//System.out.println("FIREWORKFLOW_JDBC:" + sql.toString());
				}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<IWorkItem> findTodoWorkItemsByProcessInstanceId(final String actorId, final String processInstanceId)
	{
		...
		{
			logger.debug("FIREWORKFLOW_JDBC:" + workItemSql.toString());
			logger.debug("Parameters:"+actorId+","+processInstanceId);
			//System.out.println("FIREWORKFLOW_JDBC:" + sql.toString());
		}
	...
	/**
	 * 通过processInstanceId查询流程实例下所有待办信息
	 * @Title findTodoWorkItemsByProcessInstanceId
	 * @param processInstanceId
	 * @return List<IWorkItem>
	 * @author LiQiang
	 * @date 2014-2-18
	 */
	@SuppressWarnings("unchecked")
	public List<IWorkItem> findTodoWorkItemsByProcessInstanceId(final String processInstanceId)
	{
		...
		List<IWorkItem> l = (List<IWorkItem>) super.getJdbcTemplate().query(workItemSql,
				new Object[] { processInstanceId }, new WorkItemRowMapper());
		if (l == null)
		{
			return null;
		}
	...
	public List<IWorkItem> findTodoWorkItems(final String actorId, final String processId, final String taskId)
	{
		...
		{
			logger.debug("FIREWORKFLOW_JDBC:" + workItemSql.toString());
			logger.debug("Parameters:"+actorId+","+ processId+","+ taskId);
			//System.out.println("FIREWORKFLOW_JDBC:" + sql.toString());
		}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<IWorkItem> findTodoWorkItemsByWithdraw(final String actorId, final String processInstanceId, final String taskId)
	{
		...
		String workItemSql = " select a.* from t_ff_rt_workitem a,t_ff_rt_taskinstance b where a.taskinstance_id=b.id and (a.state="
				...
				+ " or a.state="
				+ IWorkItem.RUNNING
				+ " ) and actor_id=? and b.processinstance_id=? and task_id=?  ";

		if (show_sql)
		{
			logger.debug("FIREWORKFLOW_JDBC:" + workItemSql.toString());
			logger.debug("Parameters:"+actorId+","+ processInstanceId+","+ taskId);
			//System.out.println("FIREWORKFLOW_JDBC:" + sql.toString());
		}
		
		List<IWorkItem> l = (List<IWorkItem>) super.getJdbcTemplate().query(workItemSql,
				new Object[] { actorId, processInstanceId, taskId }, new WorkItemRowMapper());
		if (l == null)
		{
			return null;
		}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<IWorkItem> findTodoWorkItemsByProcessId(final String actorId, final String processId)
	{
		if (actorId == null || actorId.trim().equals(""))
		{
			throw new NullPointerException("工单操作员（actorId）不能为空！");
		...
		String workItemSql = " select a.*,c.buz_code from t_ff_rt_workitem a,t_ff_rt_taskinstance b,t_ff_rt_processinstance c" +
				" where a.taskinstance_id=b.id and b.PROCESSINSTANCE_ID=c.id and (a.state="
				+ IWorkItem.INITIALIZED
				+ " or a.state="
				+ IWorkItem.RUNNING
				+ " ) and actor_id=? and c.process_id=?";

		if (show_sql)
		{
			logger.debug("FIREWORKFLOW_JDBC:" + workItemSql.toString());
			logger.debug("Parameters:"+actorId+","+processId);
			//System.out.println("FIREWORKFLOW_JDBC:" + sql.toString());
		}
		
		List<IWorkItem> l = (List<IWorkItem>) super.getJdbcTemplate().query(workItemSql,
				new Object[] { actorId, processId }, new WorkItemWithBuzRowMapper());
...
//		return l;
	...
	public void suspendProcessInstance(final ProcessInstance processInstance)
	{
		...
		{
			logger.debug("FIREWORKFLOW_JDBC:" + sql.toString());
			logger.debug("Parameters:"+processInstance.getId());
			//System.out.println("FIREWORKFLOW_JDBC:" + sql.toString());
		}
		...
		if (show_sql)
		{
			logger.debug("FIREWORKFLOW_JDBC:" + sql2.toString());
			logger.debug("Parameters:"+processInstance.getId());
			//System.out.println("FIREWORKFLOW_JDBC:" + sql.toString());
		}
	...
	public void abortProcessInstance(final ProcessInstance processInstance)
	{
		...
		if (show_sql)
		{
			logger.debug("FIREWORKFLOW_JDBC:" + processSql.toString());
			logger.debug("Parameters:"+getSqlDateTime(now)+","+ processInstance.getId());
			//System.out.println("FIREWORKFLOW_JDBC:" + sql.toString());
		}
		...
		if (show_sql)
		{
			logger.debug("FIREWORKFLOW_JDBC:" + taskSql.toString());
			logger.debug("Parameters:"+getSqlDateTime(now)+","+ processInstance.getId());
			//System.out.println("FIREWORKFLOW_JDBC:" + sql.toString());
		}
		...
		if (show_sql)
		{
			logger.debug("FIREWORKFLOW_JDBC:" + tokenSql.toString());
			logger.debug("Parameters:"+processInstance.getId());
			//System.out.println("FIREWORKFLOW_JDBC:" + sql.toString());
		}
	...
	public void saveOrUpdateProcessInstanceTrace(ProcessInstanceTrace processInstanceTrace)
	{
		...
		if (processInstanceTrace.getId() == null)
		{
			...
			{
				logger.debug("FIREWORKFLOW_JDBC:" + sql.toString());
				logger.debug("Parameters:"+processInstanceTraceId+","+ processInstanceTrace.getProcessInstanceId()+","+
		...
		else
		{
			...
			{
				logger.debug("FIREWORKFLOW_JDBC:" + sql.toString());
				logger.debug("Parameters:"+processInstanceTrace.getProcessInstanceId()+","+ processInstanceTrace.getStepNumber()+","+
	...
	public List<ProcessInstanceVar> findProcessInstanceVariable(final String processInstanceId)
	{
		...
		if (show_sql)
		{
			logger.debug("FIREWORKFLOW_JDBC:" + varSql.toString());
			logger.debug("Parameters:"+processInstanceId);
			//System.out.println("FIREWORKFLOW_JDBC:" + sql.toString());
		}
	...
	public ProcessInstanceVar findProcessInstanceVariable(String processInstanceId, String name)
	{
		...
		{
			logger.debug("FIREWORKFLOW_JDBC:" + varSql.toString());
			logger.debug("Parameters:"+processInstanceId+","+name);
			//System.out.println("FIREWORKFLOW_JDBC:" + sql.toString());
		}
	...
	public void updateProcessInstanceVariable(ProcessInstanceVar var)
	{
		...
		{
			logger.debug("FIREWORKFLOW_JDBC:" + sql.toString());
			logger.debug("Parameters:"+object2String(var.getValue())+","+ var.getProcessInstanceId()+","+ var.getName());
			//System.out.println("FIREWORKFLOW_JDBC:" + sql.toString());
		}
	...
	public void saveProcessInstanceVariable(ProcessInstanceVar var)
	{
		...
		{
			logger.debug("FIREWORKFLOW_JDBC:" + sql.toString());
			logger.debug("Parameters:"+var.getProcessInstanceId()+","+ var.getName()+","+ object2String(var.getValue()));
			//System.out.println("FIREWORKFLOW_JDBC:" + sql.toString());
		}
	 ...
	 /**
     * 根据年份查询当年节假日
     * findWfVacationByYear
     * @param year
     * @return List<WfVacation>
     * @author LiQiang
     * @date 2014-7-25
     */
	@SuppressWarnings("unchecked")
	public List<WfVacation> findWfVacationByYear(String year) {
		
		String varSql = "select * from wf_vacation where years=? order by vacation_date";
		
		if (show_sql)
		{
			logger.debug("FIREWORKFLOW_JDBC:" + varSql.toString());
			logger.debug("Parameters:"+year);
			//System.out.println("FIREWORKFLOW_JDBC:" + sql.toString());
		}
		
		List<WfVacation> list =  (List<WfVacation>)super.getJdbcTemplate().query(varSql,new Object[] { year }, new WfVacationRowMapper());
...
//	 */
//	public String getBuzCodeForWorkitem(final String itemid)
//	{
//		Object ob = super.getJdbcTemplate().queryForObject(
//				"select c.buz_code from t_ff_rt_workitem a,t_ff_rt_taskinstance b,t_ff_rt_processinstance c" +
//				" where a.taskinstance_id=b.id and b.PROCESSINSTANCE_ID=c.id and a.id=?",
//				new Object[] { itemid },new SingleColumnRowMapper());
//		return (String) ob;
//	}

	
//	public FlowDataOfBuz getFlowDataByBuzCode(String buzCode) {
//		String sql = "select w.* from t_ff_rt_workitem w, t_ff_rt_taskinstance  t, t_ff_rt_processinstance p "+
//		"where p.id = t.processinstance_id and w.taskinstance_id = t.id  and p.buz_code = ?  and w.state < 5";
//		List<IWorkItem> toDoWorkItems = (List<IWorkItem>) super.getJdbcTemplate().query(sql, new Object[] { buzCode }, new WorkItemRowMapper());
//		List<String> toDoMenList = new ArrayList<String>();
//		List<String> toDoItemList = new ArrayList<String>();
//		for(int i=0; i<toDoWorkItems.size(); i++){
//			IWorkItem w = toDoWorkItems.get(i);
//			toDoMenList.add(w.getActorId());
//			toDoItemList.add(w.getId());
//		}
//		FlowDataOfBuz fd = new FlowDataOfBuz();
//		try{
//			if(toDoWorkItems==null || toDoWorkItems.size()==0){
//				return null;
//			}else{
//				fd.setTodomen(toDoMenList.toString().substring(1, toDoMenList.toString().length()-1).replace(" ", "").concat(","));
//				fd.setTodoitem(toDoItemList.toString().substring(1, toDoItemList.toString().length()-1).replace(" ", ""));
//				return fd;
//			}
//		}catch(Exception e){
//			return null;
